...
class Contests(commands.Cog):
    ...
    async def ranklist(self, ctx, contest_id: int, *handles: str):
        """Shows ranklist for the contest with given contest id. 
        """
        handles = await cf_common.resolve_handles(ctx, self.member_converter, handles, maxcnt=10, default_to_all_server=True)
        # await ctx.send(f"{str(handles)}")
